...
const { Filesystem } = _capacitor_core__WEBPACK_IMPORTED_MODULE_10__["Plugins"];
let AnimalWorklistPage = class AnimalWorklistPage {
    constructor(modalController, plt, file, fileOpener, alertCtrl, http, route, router, navCtrl) {
    ...
    loadData() {
        ...
        data = this.http.get('http://10.136.137.223:18080/ords/pathlimsreports/v_nec_int_animal/?q=%7B%22path_request_id%22:%22' + this.pathrequestid + '%22,%22procedure_id%22:' + this.procedureid + '%7D')
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_11__["map"])((response) => response.items)); // map to use the items key!
        data.subscribe(data => {
            this.items = data;
            console.log(data);
        });
